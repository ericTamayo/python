...
class Register:
	...
	def Read_CalComplete(self, Lane):
		...
		calCompleteInt = int(calCompleteBin)
		if calCompleteInt == 1:
			return True
		else:
			return False  
	...
	def Read_LockDetect(self, lane):
		...
		lockDetectInt = int(lockDetectBin)
		return lockDetectInt 
	...
	def PDN_OFF(self, Lane):
		self.Object.write("y 0 %s\x0D"%Lane)
	
	def PDN_ON(self):
		self.Object.write("y 1 \x0D")
	...
	def SetTX(self):
		self.Write('0', '0x10','0xa1')
		self.Write('0', '0x1b','0x00')
	
	def SetRX(self):
		self.Write('1', '0x10','0x36')
		self.Write('1', '0x1b','0x20')
def main():
	...
	#ReTimer Debug
		...
		while (run == True):
			...
			if Board_A.Read_PassAccu("L1"):
				print ("TX1 PASS" + " CalComplete:"+ str(Board_A.Read_CalComplete("L1")))
			else:
				print ("TX1 FAIL," + " CalComplete:" + str(Board_A.Read_CalComplete("L1")))
			
			if Board_B.Read_PassAccu("L2"):
				print ("RX1 PASS," + " CalComplete:"+ str(Board_B.Read_CalComplete("L2")))
			else:
				print ("RX1 FAIL," + " CalComplete:"+ str(Board_B.Read_CalComplete("L2")))
			
			if Board_B.Read_PassAccu("L1"):
				print ("TX2 PASS," + " CalComplete:"+ str(Board_B.Read_CalComplete("L1")))
			else:
				print("TX2 FAIL," + " CalComplete:"+ str(Board_B.Read_CalComplete("L1")))
				
			if Board_A.Read_PassAccu("L2"):
				print("RX2 PASS," + " CalComplete:"+ str(Board_A.Read_CalComplete("L2")))
			else:
				print("RX2 FAIL," + " CalComplete:"+ str(Board_A.Read_CalComplete("L2")))
		...
		print ("PDN DEBUG")
		lane = input("Input lane to turn off")
		Board_A.PDN_OFF(str(lane))
		raw_input("Press Enter To Continue...")
		print("Turning On All Lanes...")
		Board_A.PDN_ON()

	elif case == 5:
		while (True):
			
			print ("Enabling All lanes...")
			Board_A.PDN_ON()
			Board_B.PDN_ON()
			print("All Lanes Enabled")

			print("Setting TX/RX Lane 1...")
			Board_A.SetTX()
			Board_B.SetRX()
			print("TX/RX Set")

			print ("Disabling Lane 2")
			Board_B.PDN_OFF("0")
			raw_input("")

			print("Reading Values For Lane 1...")
			print "\tEyeHeight Value is : %s"%Board_B.Read_EyeHeight("L2")
			print "\tGainSet Value is : %s"%Board_B.Read_GainSet("L2")
			raw_input("")

			print("Enable Lane 2")
			Board_B.PDN_ON()

			print("Setting TX/RX Lane 2...")
			Board_B.SetTX()
			Board_A.SetRX()
			print("TX/RX Set")

			print("Disableing Lane 1...")
			Board_A.PDN_OFF("0")
			raw_input("")
			print("Lane 1 Disabled")

			print("Reading Values For Lane 2...")
			print "\tEyeHeight Value is : %s"%Board_A.Read_EyeHeight("L2")
			print "\tGainSet Value is : %s"%Board_A.Read_GainSet("L2")
			runAgain = raw_input("Would you like to run again?(y)/(n)?")
